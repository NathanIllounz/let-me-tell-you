...
export default function ManualEntry({
  ...
  const initialGroups = storyToEdit?.story_groups
    ? storyToEdit.story_groups.map((sg) => sg.group_id)
    : [];
        ...
        <div className="bg-white rounded-2xl shadow-xl border border-stone-100 w-full max-w-2xl overflow-hidden flex flex-col max-h-[90vh]">
          ...
          >
            ...
            <div className="flex items-center gap-3 p-4 bg-indigo-50/50 border border-indigo-200 shadow-sm rounded-xl">
              ...
              />
              <label
                htmlFor="refineToggle"
                className="flex items-center gap-2 cursor-pointer select-none text-stone-700 font-medium"
              >
                <Sparkles className="w-4 h-4 text-indigo-500" />✨ Use Gemini to
                polish this story
              </label>
            </div>
          </form>
