...
export default function SocialCenter({ session, onClose }) {
  ...
  const fetchSocialData = async () => {
    ...
    try {
      const [profileRes, friendsRes, requestsRes] = await Promise.all([
        api.get("/friends/me", { headers }).catch((e) => {
          console.error(e);
          return { data: null };
        }),
        api.get("/friends/all", { headers }).catch((e) => {
          console.error(e);
          return { data: [] };
        }),
        api.get("/friends/pending", { headers }).catch((e) => {
          console.error(e);
          return { data: [] };
        }),
      ]);
  ...
  const handleSendRequest = async (e) => {
    ...
    if (!searchHandle.includes("#")) {
      setActionError(t("social.invalid_handle"));
      return;
    }

    const [username, tag] = searchHandle.split("#");
    if (!username || !tag) {
      setActionError(t("social.invalid_format"));
      return;
    }
    ...
    try {
      const res = await api.post(
        "/friends/request",
        { username: username.toLowerCase().trim(), tag: tag.trim() },
        {
      ...
      setActionError(
        err.response?.data?.detail || "Could not send friend request.",
      );
    }
  };
  ...
  const handleRemoveFriend = async (friendId) => {
    if (!window.confirm(t("social.remove_confirm"))) return;
    try {
      ...
      <div className="bg-[#FDFBF7] rounded-2xl shadow-2xl border border-[#D6C2A8] w-full max-w-lg overflow-hidden flex flex-col h-[600px] max-h-[90vh]">
        ...
        <div className="flex items-center justify-between p-6 border-b border-[#D6C2A8] bg-[#F3E1CE]/80">
          <div className="flex items-center gap-3">
            ...
            <h2 className="text-xl font-bold text-[#4A0E17] font-serif">
              {t("social.title")}
            </h2>
          </div>
          <button
            onClick={onClose}
            className="p-2 text-[#8C7A6B] hover:text-[#4A0E17] hover:bg-[#E5DACD] rounded-full transition-colors"
          >
            <X className="w-5 h-5" />
          </button>
        </div>
        ...
        <div className="px-6 py-4 bg-gradient-to-r from-[#3E1519] to-[#2B0E11] text-[#E5DACD] flex items-center justify-between shadow-inner">
          <div className="flex flex-col">
            <span className="text-[#c9a174] text-xs font-bold uppercase tracking-wider mb-1">
              {t("social.your_handle")}
            </span>
            {myProfile ? (
        ...
        <div className="flex border-b border-[#D6C2A8] p-2 gap-2 bg-[#FDFBF7]">
          ...
          >
            {t("social.tab_requests")}
            {requests.length > 0 && (
              <span className="bg-[#4A0E17] text-white text-[10px] px-1.5 py-0.5 rounded-full">
                {requests.length}
              </span>
            )}
          </button>
        ...
        <div className="flex-1 overflow-y-auto p-6 bg-[#FDFBF7]">
          {loading ? (
            <div className="flex justify-center items-center h-full text-[#8C7A6B] font-serif italic">
              {t("social.loading_graph")}
            </div>
          ) : (
            <>
              {activeTab === "friends" && (
                <div className="space-y-3">
                  {friends.length === 0 ? (
                    <p className="text-center text-[#8C7A6B] text-sm italic mt-10">
                      {t("social.no_friends")}
                    </p>
                  ) : (
                    friends.map((friend) => (
                      <div
                        key={friend.id}
                        className="flex items-center justify-between p-4 bg-white border border-[#D6C2A8] shadow-sm rounded-xl hover:shadow-md transition-shadow"
                      >
                        <div className="flex items-center gap-3">
              ...
              {activeTab === "requests" && (
                <div className="space-y-3">
                  {requests.length === 0 ? (
                    <p className="text-center text-[#8C7A6B] text-sm italic mt-10">
                      {t("social.no_requests")}
                    </p>
                  ) : (
                    requests.map((req) => (
                      <div
                        key={req.request_id}
                        className="flex items-center justify-between p-4 bg-white border border-[#D6C2A8] shadow-sm rounded-xl"
                      >
                        <div className="flex flex-col">
                          <span className="font-bold text-[#4A3D33]">
                            {req.sender_username}
                          ...
                          <span className="text-[10px] text-[#8C7A6B] uppercase font-bold tracking-wider mt-1">
                            {t("social.wants_to_connect")}
                          </span>
                        </div>
                        <div className="flex gap-2">
                          <button
                            onClick={() => handleAccept(req.request_id)}
                            className="p-2 bg-[#F3EBE1] text-[#4A0E17] hover:bg-[#c9a174] hover:text-white rounded-lg transition-colors border border-[#D6C2A8]"
                          ...
                          <button
                            ...
                            className="p-2 bg-[#FDFBF7] text-[#8C7A6B] hover:bg-red-50 hover:text-red-600 rounded-lg transition-colors border border-[#D6C2A8]"
              ...
              {activeTab === "add" && (
                <div className="space-y-6">
                  <div className="bg-white p-6 rounded-xl border border-[#D6C2A8] shadow-sm">
                    <h3 className="font-bold text-[#4A0E17] font-serif text-lg mb-2">
                      {t("social.add_title")}
                    </h3>
                    ...
                    <form onSubmit={handleSendRequest} className="flex gap-2">
                      <div className="relative flex-1">
                        ...
                        <input
                          ...
                          placeholder={t("social.handle_placeholder")}
                        />
                      </div>
                      <button
                        type="submit"
                        className="px-5 py-3 bg-[#3E1519] text-[#E5DACD] rounded-xl font-bold hover:bg-[#561C24] transition-colors shadow-sm flex items-center gap-2"
                      >
                  ...
                  {actionError && (
                    <div className="p-3 bg-red-50 border border-red-100 text-red-600 text-sm font-medium rounded-lg text-center shadow-inner">
                      {actionError}
                    </div>
                  )}
                  {actionMessage && (
                    <div className="p-3 bg-[#F3EBE1] border border-[#c9a174] text-[#4A0E17] text-sm font-bold rounded-lg text-center flex items-center justify-center gap-2 shadow-inner">
                      <Check className="w-4 h-4 text-[#c9a174]" />{" "}
                      {actionMessage}
                    </div>
                  )}
                </div>
