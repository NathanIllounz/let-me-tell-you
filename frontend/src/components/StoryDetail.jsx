...
export default function StoryDetail({
  ...
  const [activeTrack, setActiveTrack] = useState(
    hasNarratorAudio ? "narrator" : "original",
  );

  const currentAudioSrc =
    activeTrack === "narrator"
      ? story.refined_audio_path
      : hasOriginalAudio
        ? story.audio_path
        : null;
  ...
  const [narratorGender, setNarratorGender] = useState(
  ...
  const handleGenerateNarrator = async () => {
    ...
    try {
      const res = await api.post(`/stories/${story.id}/generate-audio`, null, {
        params: { user_id: session.user.id, gender: narratorGender },
      });
      ...
      <div className="max-w-3xl mx-auto pb-20">
          ...
          <div className="flex flex-wrap items-center justify-center gap-3 mb-6 text-xs uppercase tracking-[0.2em] font-medium text-stone-400">
            ...
            {story.language && (
              <>
                ...
                <p className="px-2 py-0.5 rounded bg-stone-100 text-stone-500 border border-stone-200">
                  {story.language}
                </p>
              </>
            )}
          ...
          <div className="mt-12 pt-10 border-t border-stone-200/60 max-w-xl mx-auto">
            {hasOriginalAudio && hasNarratorAudio && (
              <div className="flex justify-center mb-6 bg-stone-100 p-1 rounded-full items-center max-w-xs mx-auto">
                ...
                >
                  <Sparkles className="w-4 h-4 inline-block mr-1 -mt-1" />{" "}
                  Narrator
                </button>
              </div>
            )}
            ...
            {currentAudioSrc ? (
              <div className="bg-white border border-stone-200 shadow-sm rounded-2xl p-5 mb-6 animate-in fade-in zoom-in-95 duration-300">
                ...
                <div className="flex items-center gap-3 mb-4">
                  <span className="text-xs font-medium text-stone-400 w-10 text-right">
                  ...
                  />
                  <span className="text-xs font-medium text-stone-400 w-10">
                    {formatTime(duration)}
                  </span>
                </div>
        ...
        >
            ...
            <div className="absolute -top-12 right-0 opacity-0 group-hover:opacity-100 transition-opacity">
              <button
                onClick={() => {
                  ...
                  if (!isEditingText)
                    setEditedContent(story.refined_story || "");
                }}
          ...
          {isEditingText ? (
            ...
            <p
              className={`font-serif leading-loose text-stone-700 whitespace-pre-wrap text-[1.15rem] sm:text-[1.25rem] selection:bg-indigo-100 selection:text-indigo-900 first-letter:text-6xl first-letter:font-bold first-letter:text-stone-800 tracking-wide ${story.language === "Hebrew" ? "first-letter:float-right first-letter:ml-3 text-right" : "first-letter:float-left first-letter:mr-3 text-justify"}`}
            >
              {story.refined_story ||
                "No text content available for this memory."}
            </p>
          )}
        </article>
