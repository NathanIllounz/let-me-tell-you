  ...
  const handleDirectInvite = async (groupIdArg) => {
    ...
    try {
      const res = await api.post(
        `/groups/${groupIdArg}/invite-friend`,
        {
      ...
      setInviteMessage({
        type: "error",
        text: err.response?.data?.detail || "Failed to invite.",
      });
    } finally {
      setLoading(false);
        ...
        {createdGroup ? (
          <div className="p-8 flex flex-col items-center text-center">
            ...
            <h3 className="text-2xl font-bold text-stone-800 mb-2">
              {t("groups.create_success", { name: createdGroup.name })}
            </h3>
            <p className="text-stone-500 mb-6">{t("groups.share_code")}</p>
              ...
              <div className="w-full text-left bg-white border border-indigo-100 p-4 rounded-xl shadow-sm">
                ...
                <div className="flex gap-2">
                  <select
                    ...
                    disabled={loading}
                  >
                    <option value="">{t("groups.select_friend")}</option>
                    {friends.map((f) => (
                ...
                {inviteMessage && (
                  <p
                    className={`text-xs mt-2 font-bold ${inviteMessage.type === "error" ? "text-rose-500" : "text-emerald-500"}`}
                  >
                    {inviteMessage.text}
                  </p>
                )}
            ...
            <div className="p-6">
              ...
              {activeTab === "join" ? (
                <form onSubmit={handleJoin} className="flex flex-col gap-4">
                  <div>
                    ...
                    <input
                      ...
                      maxLength={6}
                      className="w-full text-center text-2xl tracking-widest font-mono p-3 border border-stone-200 rounded-lg focus:ring-2 focus:ring-indigo-400 focus:border-indigo-400 outline-none transition-shadow uppercase bg-white placeholder:text-stone-300"
                      placeholder="XXXXXX"
                      disabled={loading}
                    />
                  </div>
                  ...
                  >
                    {loading ? t("groups.joining") : t("groups.join_button")}
                  </button>
                </form>
              ) : activeTab === "create" ? (
                <form onSubmit={handleCreate} className="flex flex-col gap-4">
                  ...
                  >
                    {loading ? t("groups.creating") : t("groups.create_button")}
                  </button>
                </form>
              ) : (
                <div className="flex flex-col gap-4">
                  <div>
                    ...
                    >
                      <option value="">{t("groups.choose_circle")}</option>
                      {groups.map((g) => (
                    ...
                    <div className="mt-4 flex flex-col gap-6 animate-in fade-in duration-300">
                      <div className="bg-stone-50 border border-stone-200 rounded-xl p-4 w-full flex items-center justify-between shadow-inner">
                        <span className="text-2xl font-mono tracking-widest font-bold text-indigo-700">
                          {groups.find((g) => g.id === selectedManageGroupId)
                            ?.invite_code || "CODE"}
                        </span>
                        <button
                          onClick={() =>
                      ...
                      {friends.length > 0 ? (
                        <div className="w-full text-left bg-white border border-stone-200 p-4 rounded-xl shadow-sm">
                          ...
                          <div className="flex gap-2">
                            ...
                            <button
                              onClick={() =>
                                handleDirectInvite(selectedManageGroupId)
                              }
                              disabled={!selectedFriend || loading}
