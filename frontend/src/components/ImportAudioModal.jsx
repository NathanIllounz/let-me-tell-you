...
import { Upload, X, Loader2, FileAudio, AlertCircle, ImagePlus, Sparkles } from 'lucide-react';
import api from '../api';

export default function ImportAudioModal({ session, groups, onClose, onSaveSuccess }) {
  ...
  const uploadAudio = async () => {
    ...
    try {
      ...
      setStatus('error');
      setErrorMessage(err.response?.data?.detail || "Failed to process your legacy memory. Ensure the file is a valid audio format.");
    }
  };
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/40 backdrop-blur-md animate-in fade-in duration-200">
      <div className="bg-white rounded-3xl shadow-2xl border border-stone-100 w-full max-w-md overflow-hidden relative">
        ...
        <div className="p-8 flex flex-col items-center mt-2">
          ...
          <h2 className="text-2xl font-bold text-[#4A3D33] font-serif mb-2 text-center">Import Audio</h2>
          <p className="text-stone-500 mb-6 text-sm leading-relaxed text-center">Upload family recordings, vintage tapes, or any MP3/WAV file.</p>

          {(status === 'idle' || status === 'error') && (
            <div className="w-full mb-6 space-y-5 max-h-[45vh] overflow-y-auto px-2 pb-2">

              <div>
                <label className="block text-sm font-medium text-stone-700 mb-1">Cover Photo (Optional)</label>
              ...
              <div>
                ...
                <div className="flex items-center gap-2 w-full p-2.5 border border-stone-300 shadow-sm rounded-lg bg-stone-50 relative">
                  ...
                  <span className="text-sm text-stone-600 truncate flex-1">{file ? file.name : 'Choose an MP3 or WAV file...'}</span>
                  <input 
                    type="file" 
              ...
              <div>
                <label className="block text-sm font-medium text-stone-700 mb-1">Memory Title (Optional)</label>
                <input 
                  type="text" 
                  value={title}
              ...
              <div>
                <label className="block text-sm font-medium text-stone-700 mb-1">Who can see this?</label>
              ...
              <div>
                <label className="block text-sm font-medium text-stone-700 mb-1">AI Processing</label>
                <div className="flex items-center gap-3 p-3 bg-indigo-50/50 border border-indigo-200 shadow-sm rounded-xl">
                  ...
                  />
                  <label htmlFor="refineImportToggle" className="flex items-center gap-2 cursor-pointer select-none text-stone-700 font-medium text-sm">
              ...
              <div>
                <label className="block text-sm font-medium text-stone-700 mb-1">Audio Language</label>
                <select 
                  value={language}
          ...
          {status === 'error' && (
            <div className="flex flex-col items-center justify-center gap-4 w-full animate-in slide-in-from-bottom-4 duration-300">
              ...
              <p className="text-stone-600 text-sm bg-red-50 p-4 rounded-xl border border-red-100 text-center w-full">{errorMessage}</p>
              <button 
          ...
          {status === 'processing' && (
            <div className="flex flex-col items-center justify-center gap-6 py-10 animate-in zoom-in duration-300">
              ...
              <div className="flex flex-col items-center text-center">
                <span className="font-bold text-indigo-800 text-lg">Saving Audio...</span>
                <span className="text-stone-500 text-sm mt-2 leading-relaxed">
                  {shouldRefine ? 'Our AI is transcribing and preparing your story. This takes about 10-20 seconds.' : 'Securely storing your audio in the library...'}
                </span>
              </div>
            </div>
          )}
          ...
          {status === 'success' && (
            <div className="flex flex-col items-center justify-center gap-4 py-8 animate-in zoom-in duration-300">
              ...
              <span className="font-bold text-emerald-700 text-xl font-serif">Upload Complete!</span>
            </div>
          )}
