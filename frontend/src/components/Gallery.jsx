...
export default function Gallery({
  ...
  const isGroupCreator =
    activeGroup && activeGroup.creator_id === session?.user?.id;
  ...
  const visibleStories = useMemo(() => {
    if (activeView === "my_memories") {
      ...
      return allStories.filter((s) =>
        s.story_groups?.some((g) => g.group_id === activeView),
      );
    }
    return allStories;
  ...
  const handleDeleteGroup = async () => {
    if (
      !window.confirm(
        `Are you sure you want to delete the circle "${activeGroup.name}"? This removes it for all members.`,
      )
    )
      return;
    try {
  ...
  const handleRefreshCode = async () => {
    try {
      const res = await api.post(`/groups/${activeGroup.id}/refresh_invite`, {
        user_id: session.user.id,
      });
  ...
  const handleDeleteStory = async (story) => {
    const isConfirmed = window.confirm(
      `Are you sure you want to delete "${story.title || "Untitled Memory"}"?`,
    );
      ...
      <div className="max-w-6xl mx-auto py-10 px-6">
        <div className="flex justify-between items-center mb-10">
          <div>
            <h2 className="text-4xl font-bold text-[#4A0E17] font-serif mb-2">
              {activeView === "my_memories"
                ? t("nav.my_memories")
                : activeGroup
                  ? activeGroup.name
                  : t("nav.your_library") || "Your Library"}
            </h2>
            <p className="text-lg text-[#3E1519]/70 font-medium font-serif italic">
              {activeView === "my_memories"
                ? t("nav.my_memories_desc") ||
                  "Quiet reflections and private tales."
                : activeGroup
                  ? t("nav.group_desc", { name: activeGroup.name }) ||
                    `Shared stories with ${activeGroup.name}.`
                  : t("nav.library_desc") ||
                    "A growing collection of family history."}
            </p>
            {activeGroup && (
              <p className="text-[#3E1519]/50 text-sm mt-3 font-semibold uppercase tracking-widest">
                Circle Invite Code:{" "}
                <span className="font-mono font-bold text-[#4A0E17] bg-[#c9a174]/20 px-2 py-0.5 rounded">
            ...
            {isGroupCreator && (
              <>
                <button
                  ...
                  className="p-2 bg-white border border-[#E5DACD] hover:bg-[#F3EBE1] hover:text-[#4A3D33] text-[#8C7A6B] rounded-lg shadow-sm transition-colors"
                  title="Edit Name"
                >
                ...
                <button
                  ...
                  className="p-2 bg-white border border-[#E5DACD] hover:bg-[#F3EBE1] hover:text-indigo-600 text-[#8C7A6B] rounded-lg shadow-sm transition-colors"
                ...
                <button
                  ...
                  className="p-2 bg-white border border-[#E5DACD] hover:bg-red-50 hover:text-red-600 text-[#8C7A6B] rounded-lg shadow-sm transition-colors"
                  title="Delete Circle"
                >
        ...
        {loading ? (
          <div className="flex justify-center items-center py-32 animate-fade-in-up">
            ...
            <span className="ml-4 text-[#4A0E17] text-xl font-medium tracking-wide font-serif">
              {t("nav.organizing")}
            </span>
          </div>
          ...
          <div className="flex flex-col gap-16 mt-6">
            {monthKeys.map((monthKey, idx) => (
              <div
                key={monthKey}
                className={`relative pt-4 animate-fade-in-up delay-${Math.min((idx + 1) * 100, 600)}`}
              >
                ...
                {/* Books Container */}
                <div
                  className={`grid grid-cols-2 md:grid-cols-4 lg:grid-cols-5 gap-6 px-4 relative z-10 pb-6 items-end ${easyMode ? "md:grid-cols-3 lg:grid-cols-4 gap-8" : ""}`}
                >
                  {/* Capture Modules on the first shelf */}
                  {idx === 0 && (
                    <>
                      <div
                        className={`w-full max-w-[200px] mb-2 aspect-[2/3] ${easyMode ? "p-8" : "p-6"} bg-[#3E1519] rounded-r-xl rounded-l-[4px] border-l-4 border-l-[#c9a174] flex flex-col justify-center items-center text-center gap-4 hover:-translate-y-2 transition-all duration-300 cursor-pointer shadow-lg hover:shadow-xl group book-entry relative overflow-hidden`}
                      ...
                      >
                        ...
                        <div className="w-12 h-12 bg-[#2B0E11] rounded-xl flex items-center justify-center shadow-inner mb-2 group-hover:bg-[#c9a174] transition-colors duration-300">
                          <Mic
                            className={`${easyMode ? "w-8 h-8" : "w-6 h-6"} text-[#E5DACD] group-hover:text-[#2B0E11] transition-colors`}
                          />
                        </div>
                        <h3
                          className={`${easyMode ? "text-2xl" : "text-lg"} font-bold text-white font-serif leading-tight z-10`}
                        >
                          {easyMode
                            ? t("gallery.record_audio")
                            : t("gallery.capture_voice")}
                        </h3>
                      </div>

                      <div
                        className={`w-full max-w-[200px] mb-2 aspect-[2/3] ${easyMode ? "p-8" : "p-6"} bg-gradient-to-br from-[#c9a174] to-[#8F7C6B] rounded-r-xl rounded-l-[4px] border-l-4 border-l-[#2B0E11] flex flex-col justify-center items-center text-center gap-4 hover:-translate-y-2 transition-all duration-300 cursor-pointer shadow-lg hover:shadow-xl group book-entry relative overflow-hidden`}
                      ...
                      >
                        ...
                        <div className="w-12 h-12 bg-white/20 rounded-xl flex items-center justify-center shadow-inner mb-2 group-hover:bg-white transition-colors duration-300 backdrop-blur-md">
                          <PenTool
                            className={`${easyMode ? "w-8 h-8" : "w-6 h-6"} text-white group-hover:text-[#8F7C6B] transition-colors`}
                          />
                        </div>
                        <h3
                          className={`${easyMode ? "text-2xl" : "text-lg"} font-bold text-white font-serif leading-tight drop-shadow-sm z-10`}
                        >
                          {easyMode
                            ? t("gallery.write_story")
                            : t("gallery.draft_journal")}
                        </h3>
                      </div>

                      <div
                        className={`w-full max-w-[200px] mb-2 aspect-[2/3] ${easyMode ? "p-8" : "p-6"} bg-[#2B0E11] rounded-r-xl rounded-l-[4px] border-l-4 border-l-[#8F7C6B] flex flex-col justify-center items-center text-center gap-4 hover:-translate-y-2 transition-all duration-300 cursor-pointer shadow-lg hover:shadow-xl group book-entry relative overflow-hidden`}
                      ...
                      >
                        ...
                        <div className="w-12 h-12 bg-[#3E1519] rounded-xl flex items-center justify-center shadow-inner mb-2 group-hover:bg-[#8F7C6B] transition-colors duration-300">
                          <FileAudio
                            className={`${easyMode ? "w-8 h-8" : "w-6 h-6"} text-[#E5DACD] group-hover:text-white transition-colors`}
                          />
                        </div>
                        <h3
                          className={`${easyMode ? "text-2xl" : "text-lg"} font-bold text-white font-serif leading-tight z-10`}
                        >
                          {easyMode
                            ? t("gallery.import_audio")
                            : t("gallery.import_legacy")}
                        </h3>
                      </div>
                    </>
                  )}
          ...
          <div className="mt-10 animate-fade-in-up p-12 bg-[#3E1519] rounded-2xl border border-white/10 shadow-2xl flex flex-col items-center text-center max-w-4xl mx-auto relative overflow-hidden">
            ...
            <h2 className="text-5xl font-bold text-white mb-6 font-serif relative z-10 drop-shadow-md">
              {t("gallery.empty_title")}
            </h2>
            <p className="text-xl text-[#E5DACD] mb-16 max-w-xl leading-relaxed relative z-10">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6 w-full justify-center max-w-4xl px-4 relative z-10">
              ...
              >
                ...
                <span className="font-bold text-white group-hover:text-[#1e1a17] text-xl font-serif transition-colors">
                  {t("gallery.record_audio")}
                </span>
              </button>
              ...
              >
                ...
                <span className="font-bold text-white text-xl font-serif">
                  {t("gallery.write_story")}
                </span>
              </button>
        ...
        {!loading && (
          <div className="mt-32 mb-8 pt-12 border-t border-[#3E1519]/10 text-center flex flex-col items-center animate-fade-in-up delay-700">
            ...
            <p className="font-serif text-[#4A0E17]/70 italic tracking-wide text-xl">
              {t("gallery.safe_legacy")}
            </p>
            <p className="text-xs text-[#4A0E17]/40 uppercase tracking-widest mt-6 font-bold">
              Let Me Tell You © {new Date().getFullYear()}
            </p>
          </div>
        )}
      </div>
